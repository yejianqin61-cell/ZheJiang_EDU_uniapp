...
展示已生成试卷的完整内容，包括题目、选项、分值。答案与解析默认折叠，点击展开。

**Business Rules**:
- BR-006: 预览状态下不展示答案（折叠态）
- BR-007: 导出时携带答案（独立分页）
...
**Business Rules**:
- BR-010: 导出必须在15秒内完成
- BR-011: 生成的文件上传至COS，返回临时下载链接（有效期24小时）
...
管理员上传题库资料文件，系统自动判断文件类型并路由至对应解析流程。
...
**Business Rules**:
- BR-103: OCR处理时间 ≤ 60秒/文件
- BR-104: OCR识别成功率目标 ≥ 90%
...
系统自动分析题目内容，识别学科、年级、知识点。

**知识点处理**:
1. 对题目内容生成Embedding向量
2. 在知识点库中做语义相似度检索
3. 匹配成功(相似度 ≥ 阈值) → 直接关联
4. 匹配失败 → 自动创建新知识点，自动归入对应学科/年级分类
...
**Business Rules**:
- BR-111: 删除操作为软删除，标记 is_deleted = true
- BR-112: 已删除题目不再参与组卷检索
...
**Business Rules**:
- BR-114: 仅 APPROVED 状态题目可被组卷检索
- BR-115: REJECTED 题目保留30天后自动清除
