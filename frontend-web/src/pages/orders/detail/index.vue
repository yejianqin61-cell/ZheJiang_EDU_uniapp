...
async function handleExport() {
  ...
  try { ElMessage.info('正在生成试卷...'); const d=await api.post(`/papers/${order.value.paperId}/export/docx`); dlUrl=d?.downloadUrl??''; ElMessage.success('导出成功'); if(dlUrl)window.open(dlUrl,'_blank') } catch { ElMessage.error('导出失败') } finally { exporting.value=false }
}
...
<template>
  <div class="detail-page" v-if="!loading && order">
    ...
    <template v-else>
      ...
      <div class="page-card mt-md"><h3>物流状态</h3><el-timeline class="mt-md"><el-timeline-item v-for="(s,i) in PRINT_FLOW" :key="s" :timestamp="order.printStatusLog?.find(x=>x.status===s)?.time" :type="getStep()>=i?'primary':'info'">{{ psLabels[s] }}</el-timeline-item></el-timeline><el-timeline v-if="!order.printStatus" class="mt-md"><el-timeline-item type="warning">待处理 — 等待管理员处理</el-timeline-item></el-timeline></div>
    </template>
  </div>
