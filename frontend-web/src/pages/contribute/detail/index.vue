...
import { ref, onMounted } from 'vue'; import { useRoute } from 'vue-router'; import api from '@/api/index'
const route = useRoute(); const item = ref<any>(null); const loading = ref(true)
const stLabels: Record<string,string> = { pending_review:'待审核', approved:'已入库', rejected:'已驳回' }
onMounted(async () => { try { item.value = await api.get(`/contributions/${route.params.id}`) } catch {} finally { loading.value = false } })
</script>

<template>
  <div class="detail-page">
    <div class="breadcrumb"><router-link to="/">首页</router-link><span class="breadcrumb__separator">›</span><router-link to="/contribute">我的贡献</router-link><span class="breadcrumb__separator">›</span><span class="breadcrumb__current">贡献详情</span></div>
    ...
    <div v-else-if="item" class="page-card">
      ...
      <div class="info-grid mt-md">
        ...
        <div class="info-row"><span>状态</span><el-tag :type="item.status==='approved'?'success':item.status==='rejected'?'danger':'warning'" size="small">{{ stLabels[item.status]??item.status }}</el-tag></div>
        <div class="info-row"><span>题目数</span><span>{{ item.questionCount??'—' }}</span></div>
        <div class="info-row" v-if="item.reward"><span>返现金额</span><span class="amount">¥{{ (item.reward/100).toFixed(2) }}</span></div>
