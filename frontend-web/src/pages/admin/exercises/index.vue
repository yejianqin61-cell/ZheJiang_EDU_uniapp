...
import {
  ...
  type ExerciseCategoryCreatePayload,
  type ExerciseCategoryType,
  type ExerciseCategoryUpdatePayload,
  type ExerciseLesson,
} from '@/api/modules/exercise'
import type { ExercisePaper } from '@/types'
import { ElMessage, ElMessageBox } from 'element-plus'

type SyncLessonItem = ExerciseLesson & { unitName: string }
...
const examCats = ref<ExerciseCategory[]>([])
const unitPapers = ref<Record<string, ExercisePaper[]>>({})
const topicPapers = ref<Record<string, ExercisePaper[]>>({})
const examPapers = ref<Record<string, ExercisePaper[]>>({})
const syncLessons = ref<SyncLessonItem[]>([])
const syncLessonPapers = ref<Record<string, ExercisePaper[]>>({})
const activePanel = ref('unit')

const dialog = ref(false)
const dialogTitle = ref('')
const dialogForm = ref<CategoryDialogForm>({ type: 'unit', grade: '', subject: '', name: '' })
const uploadDialog = ref(false)
const uploadForm = ref({ title: '', categoryId: '', lessonId: '', file: null as File | null })
...
async function loadAll() {
  ...
  for (const category of unitCats.value) {
    unitPapers.value[category.id] = await adminListPapers(category.id)
  }

  const unitLessons: SyncLessonItem[] = []
  for (const category of unitCats.value) {
    const lessons = await adminListLessons(category.id)
    for (const lesson of lessons) {
      syncLessonPapers.value[lesson.id] = await adminListPapers(undefined, lesson.id)
    }
    unitLessons.push(...lessons.map((lesson) => ({ ...lesson, unitName: category.name })))
  }
  syncLessons.value = unitLessons

  topicCats.value = await adminListCategories({ ...params, type: 'topic' })
  for (const category of topicCats.value) {
    topicPapers.value[category.id] = await adminListPapers(category.id)
  }

  examCats.value = await adminListCategories({ ...params, type: 'exam' })
  for (const category of examCats.value) {
    examPapers.value[category.id] = await adminListPapers(category.id)
  }
}

function openNew(type: ExerciseCategoryType) {
...
function openEdit(category: ExerciseCategory) {
  ...
  dialogForm.value = {
    ...
    name: category.name,
    term: category.term ?? undefined,
    examType: category.examType ?? undefined,
  }
  dialog.value = true
}
...
async function saveCat() {
  try {
    if (dialogForm.value.id) {
      const payload: ExerciseCategoryUpdatePayload = {
        name: dialogForm.value.name,
        term: dialogForm.value.term,
      ...
      await adminCreateCategory({
        ...
        subject: dialogForm.value.subject,
        name: dialogForm.value.name,
        term: dialogForm.value.term,
        examType: dialogForm.value.examType,
      })
...
async function delCat(id: string) {
  try {
    await ElMessageBox.confirm('删除此分类将同时删除下级课时和试卷，确认？', '删除', { type: 'warning' })
...
async function openNewLesson(unitId: string) {
  try {
    const { value: name } = await ElMessageBox.prompt('课时名称', '新建课时')
    if (name) {
      await adminCreateLesson({ unitId, name })
...
async function delLesson(id: string) {
  try {
    await ElMessageBox.confirm('删除此课时将同时删除下辖试卷', '删除', { type: 'warning' })
...
function onFileChange(event: Event) {
  uploadForm.value.file = (event.target as HTMLInputElement).files?.[0] ?? null
}
...
async function doUpload() {
  ...
  try {
    ...
    if (uploadForm.value.lessonId) formData.append('lessonId', uploadForm.value.lessonId)
    await adminCreatePaper(formData)
