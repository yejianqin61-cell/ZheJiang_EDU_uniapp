...
onMounted(async () => {
  try {
    ...
    item.value = null
    ElMessage.error(getErrorMessage(error, '题目详情加载失败'))
  }
  finally {
    loading.value = false
  }
...
<template>
  <div class="detail-page">
    <div class="breadcrumb">
      ...
      <span class="breadcrumb__current">题目详情</span>
    </div>
    ...
    <div v-else-if="item" class="page-card">
      <div class="q-header">
        ...
        <el-tag size="large" class="ml-sm">{{ item.subject }} · {{ item.grade }}</el-tag>
      </div>
      <p class="q-content mt-md" v-html="renderMarkdown(item.content)"></p>
      <div v-if="item.options?.length" class="q-options mt-md">
        <div v-for="(o, i) in item.options" :key="i" class="q-option">{{ o }}</div>
      </div>
      <div class="mt-lg">
        <h4>答案</h4>
      ...
      <div v-if="item.sourceFile" class="mt-md">
        <span class="text-secondary">来源文件：</span>{{ item.sourceFile.filename }}
      </div>
      <div class="mt-lg">
        <el-button type="danger" @click="del">删除此题</el-button>
      </div>
    </div>
  </div>
</template>
