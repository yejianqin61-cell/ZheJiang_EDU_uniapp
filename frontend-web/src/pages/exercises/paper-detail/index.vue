...
const paper = ref<any>(null); const loading = ref(true)

onMounted(async () => {
  const id = route.params.id as string
  if (!id) { router.back(); return }
  try { paper.value = await getPaperDetail(id) } catch {} finally { loading.value = false }
})
...
.thumbnail-wrap{background:#fff;border-radius:$border-radius;padding:$spacing-md;box-shadow:$box-shadow-light;
  img{width:100%;display:block;border-radius:4px}
}
.thumbnail-placeholder{display:flex;flex-direction:column;align-items:center;gap:8px;padding:60px 20px;background:#fafafa;border:1px dashed #ddd;border-radius:$border-radius;span:first-child{font-size:48px}span{font-size:$font-size-sm;color:$text-color-secondary}}
